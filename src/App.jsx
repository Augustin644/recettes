...
async function ensureUserDoc(user) {
  ...
  // Toujours rafraîchir l'identité (au cas où un doc partiel aurait déjà été
  // créé par quelqu'un qui nous a suivi avant notre toute première connexion).
  await setDoc(ref_, {
...
function SettingsModal({ onClose, myUid, myProfile, onToast }) {
  ...
  const togglePrivacy = async () => {
    ...
    try {
      await setDoc(doc(db, 'users', myUid), { isPrivate: next }, { merge: true });
...
function FollowButton({ myUid, targetUid, targetIsPrivate, relation, onToast, compact, stopPropagation }) {
  ...
  const respond = async (req, accept) => {
    try {
      if (accept) {
        ...
        await setDoc(doc(db, 'users', myUid), { followersCount: increment(1) }, { merge: true });
...
function MembersDirectory({ myUid, myFollowsMap, onOpenProfile, onToast }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(qy, snap => {
      setAllUsers(snap.docs.map(d => ({ id: d.id, ...d.data() })).filter(u => u.id !== myUid));
      setLoading(false);
    }, () => setLoading(false));
    return unsub;
  }, [myUid]);
  ...
  return (
    <div>
      ...
      {loading && <div style={{ fontSize: '0.82rem', color: 'var(--text-muted)', padding: '0.8rem 0.3rem' }}>Chargement des membres…</div>}

      {!loading && visible.length === 0 && (
        <div className="empty-state" style={{ padding: '2rem 1rem' }}>
          <div className="empty-state-emoji">🔍</div>
          <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)' }}>
            {term ? 'Aucun membre ne correspond à cette recherche.' : 'Aucun autre membre pour le moment.'}
          </p>
        </div>
      )}
      ...
      {visible.map(u => (
        <div key={u.id} onClick={() => onOpenProfile(u.id)} className="member-search-row">
          <div className="profile-avatar-btn" style={{ width: 40, height: 40, fontSize: '1rem', cursor: 'pointer', flexShrink: 0 }}>
            {u.displayName?.[0]?.toUpperCase() || '?'}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: '0.88rem', fontWeight: 700, color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
              {u.displayName} {u.isPrivate && <span style={{ fontSize: '0.75rem' }}>🔒</span>}
            </div>
            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{u.followersCount || 0} abonné{(u.followersCount||0) !== 1 ? 's' : ''}</div>
          </div>
          <FollowButton myUid={myUid} targetUid={u.id} targetIsPrivate={u.isPrivate} relation={myFollowsMap[u.id]} onToast={onToast} compact stopPropagation />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="app-container">
        ...
        {/* SOUS-ONGLETS COMMUNAUTÉ : Découvrir / Abonnements / Membres */}
        {activeTab === 'public' && (
          <div className="community-subtabs-wrap">
            <div className="form-tab-row" style={{ maxWidth: 1100, margin: '0 auto' }}>
              ...
              <button type="button" onClick={() => setCommunityMode('members')} className={`form-tab-btn ${communityMode==='members'?'active':''}`}>🔎 Membres</button>
            </div>
          </div>
        )}

        {activeTab === 'public' && communityMode === 'members' ? (
          <main className="main-content">
            <MembersDirectory myUid={user.uid} myFollowsMap={myFollowsMap} onOpenProfile={setProfileViewId} onToast={addToast} />
          </main>
        ) : (
        <>
  ...
  return (
    <style>{`
      ...
      *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .follow-btn.compact {
        flex: none; padding: 7px 12px; font-size: 0.74rem; border-radius: 9px; white-space: nowrap;
      }
