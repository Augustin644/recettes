...
// ── AI ASSIST PANEL (à l'intérieur du formulaire d'ajout) ───────────────────
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(44,26,14,0.45)', backdropFilter:'blur(3px)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }} onClick={onClose}>
      <div style={{ background:'#fff', borderRadius:18, width:'100%', maxWidth:440, padding:'1.75rem', boxShadow:'0 20px 60px rgba(44,26,14,0.25)' }} onClick={e=>e.stopPropagation()}>
        <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'1.3rem', color:'#2C1A0E', marginBottom:'0.75rem' }}>Clé API Anthropic</div>
        <p style={{ fontSize:'0.85rem', color:'#8C7B6B', lineHeight:1.6, marginBottom:'1rem' }}>
          ...
          Tu peux en créer une sur <span style={{color:'#C4622D'}}>console.anthropic.com</span> (rubrique API Keys).
        </p>
        <input
          type="password"
          value={key}
...
export default function App() {
        ...
        unsub = onSnapshot(
          ...
          async (snap) => {
            if (snap.empty && !seededRef.current) {
              ...
              return; // le snapshot va se redéclencher avec les nouvelles recettes
            }
  ...
  const handleDelete = useCallback(async (id) => {
    ...
    try {
      ...
      if (recipe?.photoURL) {
        try { await deleteObject(ref(storage, `photos/${id}`)); } catch { /* photo déjà absente, tant pis */ }
      }
