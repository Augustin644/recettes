...
// ── DETAIL MODAL ──────────────────────────────────────────────────────────────
function DetailModal({ recipe, onClose, onEdit, timerCtx }) {
...
export default function App() {
  ...
  const uploadPhoto = async (id, photoFile) => {
    const sRef = ref(storage, `photos/${id}`);
  ...
  const handleSaveEdit = useCallback(async (data, photoFile) => {
    ...
    if (photoFile) {
      ...
      if (recipe.photoURL) {
        try { await deleteObject(ref(storage, `photos/${editId}`)); } catch {}
      }
  ...
  const handleDelete = useCallback(async (id) => {
    ...
    try {
      ...
      if (recipe?.photoURL) { try { await deleteObject(ref(storage, `photos/${id}`)); } catch {} }
      addToast('Recette supprimée', 'info');
      if (detailId === id) setDetailId(null);
    } catch { addToast('Erreur de suppression', 'error'); }
  }, [recipes, addToast, detailId]);
  ...
  const editRecipe = recipes.find(r => r.id === editId);
      ...
      <main style={{ maxWidth:1100, margin:'0 auto', padding:'2rem 1.5rem' }}>
        ...
        {filtered.length === 0 && syncStatus !== 'loading' && (
          <div style={{ textAlign:'center', padding:'4rem 2rem', color:'#8C7B6B' }}>
            ...
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'1.1rem', marginBottom:'0.4rem', color:'#2C1A0E' }}>Aucune recette ici</div>
            <p style={{ fontSize:'0.88rem' }}>Ajoute ta première recette !</p>
          </div>
        )}
      </main>
