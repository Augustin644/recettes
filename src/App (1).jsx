...
function ProfileMenu({ user, statusInfo, onSettings, onLogout }) {
  ...
  return (
    <div ref={ref_} style={{ position:'relative' }}>
      ...
      {open && (
        <div style={{ position:'absolute', top:'calc(100% + 8px)', right:0, background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:12, boxShadow:'0 8px 24px rgba(0,0,0,0.12)', minWidth:210, zIndex:200, overflow:'hidden' }}>
          {/* Infos utilisateur */}
          <div style={{ padding:'0.9rem 1rem', borderBottom:'1px solid var(--border)', background:'var(--accent-light)' }}>
            <div style={{ fontSize:'0.85rem', fontWeight:600, color:'var(--text-main)' }}>👤 {user?.displayName}</div>
          ...
          <button onClick={() => { onSettings(); setOpen(false); }}
            style={{ width:'100%', padding:'0.75rem 1rem', background:'none', border:'none', textAlign:'left', fontSize:'0.85rem', color:'var(--text-main)', cursor:'pointer', display:'flex', alignItems:'center', gap:'0.5rem', fontFamily:'inherit' }}
          ...
          <button onClick={() => { onLogout(); setOpen(false); }}
            style={{ width:'100%', padding:'0.75rem 1rem', background:'none', border:'none', textAlign:'left', fontSize:'0.85rem', color:'#dc2626', cursor:'pointer', display:'flex', alignItems:'center', gap:'0.5rem', fontFamily:'inherit' }}
  ...
  return (
    <div style={{ maxWidth:1100, margin:'0 auto', padding:'1.5rem 2rem' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.5rem', flexWrap:'wrap', gap:'0.75rem' }}>
        <div>
          <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'1.4rem', fontWeight:600, color:'var(--text-main)' }}>Planning repas</div>
        ...
        <button onClick={() => setShowShoppingList(true)}
          style={{ background:'var(--accent)', color:'#fff', border:'none', borderRadius:10, padding:'0.6rem 1.2rem', fontFamily:'inherit', fontWeight:600, fontSize:'0.85rem', cursor:'pointer', display:'flex', alignItems:'center', gap:'0.5rem' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(140px, 1fr))', gap:'0.75rem' }}>
        {DAYS.map(day => (
          <div key={day} style={{ background:'var(--bg-card)', border:'1.5px solid var(--border)', borderRadius:14, overflow:'hidden' }}>
            {/* Day header */}
            <div style={{ background:'var(--bg-nav)', padding:'0.5rem 0.75rem', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <span style={{ fontSize:'0.78rem', fontWeight:700, color:'var(--text-main)' }}>{day}</span>
              <button onClick={() => setAddingTo(addingTo === day ? null : day)}
                style={{ background:'var(--accent)', color:'#fff', border:'none', borderRadius:6, width:22, height:22, cursor:'pointer', fontSize:'0.9rem', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>+</button>
            </div>
            ...
            <div style={{ padding:'0.5rem', minHeight:60 }}>
              ...
              {(plan[day] || []).map((meal, i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:'0.3rem', marginBottom:'0.3rem', background:'var(--accent-light)', borderRadius:7, padding:'0.35rem 0.5rem' }}>
            ...
            {addingTo === day && (
              <div style={{ borderTop:'1px solid var(--border)', padding:'0.5rem' }}>
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher…"
                  style={{ width:'100%', padding:'0.35rem 0.5rem', border:'1px solid var(--border)', borderRadius:6, fontFamily:'inherit', fontSize:'0.75rem', outline:'none', marginBottom:'0.4rem', color:'var(--text-main)', background:'var(--bg-main)' }} />
                <div style={{ maxHeight:140, overflowY:'auto' }}>
                  {filteredRecipes.slice(0,20).map(r => (
                    <div key={r.id} onClick={() => addMeal(day, r)}
                      style={{ padding:'0.35rem 0.4rem', fontSize:'0.75rem', cursor:'pointer', borderRadius:6, color:'var(--text-main)' }}
                      onMouseEnter={e => e.currentTarget.style.background='var(--accent-light)'}
                ...
                <button onClick={() => { setAddingTo(null); setSearch(''); }}
                  style={{ width:'100%', marginTop:'0.4rem', background:'none', border:'1px solid var(--border)', borderRadius:6, padding:'0.3rem', fontSize:'0.72rem', color:'var(--text-muted)', cursor:'pointer', fontFamily:'inherit' }}>Fermer</button>
              </div>
            )}
          </div>
        ))}
      ...
      {/* Shopping List Modal */}
      {showShoppingList && (
        <div style={{ position:'fixed', inset:0, background:'rgba(31,26,23,0.4)', backdropFilter:'blur(4px)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}
          onClick={() => setShowShoppingList(false)}>
          <div style={{ background:'#fff', borderRadius:20, width:'100%', maxWidth:440, maxHeight:'88vh', overflowY:'auto', padding:'1.75rem', boxShadow:'0 20px 50px rgba(0,0,0,0.15)' }}
            onClick={e => e.stopPropagation()}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.25rem' }}>
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:'1.3rem', fontWeight:600, color:'var(--text-main)' }}>🛒 Liste de courses</div>
              <button onClick={() => setShowShoppingList(false)}
                style={{ background:'var(--bg-nav)', border:'none', borderRadius:8, width:32, height:32, cursor:'pointer', color:'var(--text-muted)', fontSize:'0.9rem' }}>✕</button>
            </div>
            ...
            {items.length === 0 ? (
              ...
              <>
                <div style={{ fontSize:'0.72rem', color:'var(--text-muted)', marginBottom:'0.75rem', textTransform:'uppercase', letterSpacing:'0.08em', fontWeight:600 }}>
                ...
                {unchecked.map(item => (
                  <div key={item.name} onClick={() => toggleCheck(item.name.toLowerCase())}
                    ...
                    {item.quantities.length > 0 && (
                      <span style={{ fontSize:'0.8rem', color:'var(--accent)', fontWeight:600 }}>{item.quantities.join(' + ')}</span>
                    )}
                  </div>
                ))}
                ...
                {checked.length > 0 && (
                  <>
                    <div style={{ fontSize:'0.72rem', color:'var(--text-muted)', margin:'1rem 0 0.5rem', textTransform:'uppercase', letterSpacing:'0.08em', fontWeight:600 }}>
                      ✓ Déjà dans le panier
                    </div>
                    {checked.map(item => (
                      <div key={item.name} onClick={() => toggleCheck(item.name.toLowerCase())}
                        style={{ display:'flex', alignItems:'center', gap:'0.75rem', padding:'0.55rem 0', cursor:'pointer', opacity:0.45 }}>
                        <div style={{ width:20, height:20, borderRadius:5, border:'2px solid var(--accent)', background:'var(--accent)', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
                ...
                <button onClick={() => setCheckedItems({})}
                  style={{ width:'100%', marginTop:'1.25rem', padding:'0.6rem', background:'none', border:'1px dashed var(--border)', borderRadius:10, fontFamily:'inherit', fontSize:'0.82rem', color:'var(--text-muted)', cursor:'pointer' }}>
                  Tout réinitialiser
                </button>
              </>
            )}
  ...
  return (
    <style>{`
      ...
      :root {
        ...
        --ios-orange:       #FF9500;
        --ios-teal:         #30B0C7;
        --ios-accent:       var(--accent, #C85329);
        --ios-grouped-bg:   #F2F2F7;
        --ios-grouped-card: #FFFFFF;
        --ios-nav-bg:       rgba(242,242,247,0.85);
        --ios-shadow:       0 1px 0 rgba(0,0,0,0.12), 0 2px 8px rgba(0,0,0,0.06);
      }
      ...
      @media (prefers-color-scheme: dark) {
        :root {
          ...
          --ios-fill2:        rgba(120,120,128,0.32);
          --ios-blue:         #0A84FF;
          --ios-red:          #FF453A;
          --ios-green:        #30D158;
          --ios-orange:       #FF9F0A;
          ...
          --ios-grouped-card: #1C1C1E;
          --ios-nav-bg:       rgba(28,28,30,0.85);
          --ios-shadow:       0 1px 0 rgba(255,255,255,0.08), 0 2px 8px rgba(0,0,0,0.4);
        }
      }
      ...
      *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .recipe-card:active { background: var(--ios-fill2); }
      ...
      .card-action-inline-btn:active { opacity: 0.7; }

      .delete-btn { width: 28px; height: 28px; color: var(--ios-red); font-size: 0.75rem; }
      .fav-btn { width: 28px; height: 28px; font-size: 0.85rem; }
      .add-btn {
        background: var(--ios-accent);
        color: #fff;
        padding: 4px 10px;
      ...
      .form-layout { max-width: 530px; padding: 20px; }
      .detail-layout { max-width: 630px; width: 100%; }
      ...
      @media (min-width: 600px) {
        .modal-drag-indicator { display: none; }
      }
      ...
      .primary-action-btn:active { opacity: 0.8; }
      ...
      /* ── VISIBILITY TOGGLE ───────────────────────────────────────── */
      .visibility-toggle-container { display: flex; gap: 8px; }
      ...
      /* ── SKETCH SHAPES (kept for emoji fallback) ─────────────────── */
      .card-illustration-sketch { display: none; }
      .sketch-shape { display: none; }
      ...
      @media (max-width: 600px) {
        .form-layout { padding: 16px; border-radius: 20px 20px 0 0; }
        .form-grid-3 { grid-template-columns: 1fr 1fr; }
        .modal-actions-drawer { flex-direction: column; }
        .modal-actions-drawer button { width: 100%; justify-content: center; }
        .tabs-container { gap: 1px; }
        .tab-btn { padding: 6px 10px; font-size: 0.75rem; }
        .timer-floating-widget { right: 12px; min-width: auto; }
